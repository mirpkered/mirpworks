...
function brandLogo() {
  return `<span class="hero-logo-frame"><img src="/assets/brand/mirpworks-logo.jpg" alt="Mirpworks — Let’s play a game." width="1200" height="500"></span>`;
}
...
function projectCard(project, compact = false) {
  return `<article class="project-card${project.featured ? " featured" : ""}${compact ? " compact" : ""}">
    ...
    <div class="card-actions"><a class="text-link" href="/projects/${project.slug}/" aria-label="View ${project.name}">View project <span aria-hidden="true">↗</span></a>${project.links?.map(link => `<a class="text-link live-link" href="${link.href}" target="_blank" rel="noopener noreferrer">${link.label} <span aria-hidden="true">↗</span></a>`).join("") ?? ""}</div>
  </article>`;
}
...
for (const project of projects) {
  pages.push({
    ...
    html: layout({
      ...
      <section class="project-body shell"><div class="project-overview"><p class="eyebrow">Overview</p><h2>${project.overview}</h2></div>${project.links?.length ? `<section class="project-links" aria-label="Live project links">${project.links.map(link => `<a class="button" href="${link.href}" target="_blank" rel="noopener noreferrer">${link.label} <span aria-hidden="true">↗</span></a>`).join("")}</section>` : ""}${project.screenshots?.length ? `<section class="project-gallery" aria-label="${project.name} screenshots"><div class="section-heading"><div><p class="eyebrow">Screenshots</p><h2>See it in play</h2></div></div><div class="screenshot-grid">${project.screenshots.map((screenshot, index) => `<figure><a href="${screenshot.src}" target="_blank" rel="noopener noreferrer" aria-label="Open full-size screenshot: ${screenshot.alt}"><img src="${screenshot.src}" alt="${screenshot.alt}" loading="lazy" decoding="async" width="${screenshot.width}" height="${screenshot.height}"></a><figcaption>${project.slug === "trivia-generator" ? ["Setup", "Text question", "Image question"][index] : ["Main menu", "Free-play puzzle", "Adventure map"][index] ?? `Screenshot ${index + 1}`}</figcaption></figure>`).join("")}</div></section>` : `<div class="media-placeholder" role="img" aria-label="Media for ${project.name} will be added as development progresses"><span aria-hidden="true">MW—${String(projects.indexOf(project) + 1).padStart(2, "0")}</span><p>Project media<br>coming later</p></div>`}${project.engine || project.presentation || project.visualDirection || project.tone ? `<section class="project-facts" aria-label="Project details"><p class="eyebrow">At a glance</p><dl>${[["Engine", project.engine], ["Presentation", project.presentation], ["Visual direction", project.visualDirection], ["Tone", project.tone]].filter(([, value]) => value).map(([label, value]) => `<div><dt>${label}</dt><dd>${value}</dd></div>`).join("")}</dl></section>` : ""}${project.gameplay ? `<section class="gameplay-details"><p class="eyebrow">Gameplay</p><h2>Characters and roles</h2><ul>${project.gameplay.map(item => `<li>${item}</li>`).join("")}</ul></section>` : ""}${project.systems?.length || project.principles?.length ? `<div class="detail-grid">${project.systems?.length ? `<section><p class="eyebrow">Major systems</p><h2>Areas in development</h2><ul>${project.systems.map(item => `<li>${item}</li>`).join("")}</ul></section>` : ""}${project.principles?.length ? `<section><p class="eyebrow">Design principles</p><h2>What guides the work</h2><ul>${project.principles.map(item => `<li>${item}</li>`).join("")}</ul></section>` : ""}</div>` : ""}${project.status ? `<aside class="development-note"><p class="eyebrow">Development status</p><p>This page reflects the project’s current public status. More detail, media, and relevant links will be added as the work is ready to share.</p></aside>` : ""}</section>`
    })
  });
}
